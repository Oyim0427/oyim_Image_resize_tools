...
if len(sys.argv) > 1:
    ...
    possible_paths = [
        os.path.join(executable_dir, "Image_HRER!"),  # 実行ファイルのディレクトリ内のImage_HRER!
...
print("🖼️  1:1 比率画像処理ツール")
print("=" * 50)
print("目標画像サイズを入力してください（例: 960、1200、1500）")
print("注意: 画像を 0_input_images フォルダに入れてください")
print()
...
while True:
    ...
    while True:
        try:
            user_input = input("目標画像サイズを入力してください: ").strip()
            if user_input:
                target_size = int(user_input)
                if target_size > 0:
                    break
                else:
                    print("❌ 0より大きい数字を入力してください！")
            else:
                print("❌ 画像サイズを入力してください！")
        except ValueError:
    ...
    print(f"✅ 目標サイズ: {target_width}x{target_height} に設定しました")
    print("長辺を維持し、短辺を拡張して正方形にします（画像内容は完全に保持されます）")

    # 画像処理を実行
    print("画像のリサイズと正方形化を開始します...")
