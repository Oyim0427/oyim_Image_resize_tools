...
if len(sys.argv) > 2:
    ...
    if base_dir is None:
        ...
        print(f"⚠️  警告: Image_HRER!フォルダが見つかりませんでした。実行ファイルのあるディレクトリを使用します: {base_dir}")
...
# 使用するディレクトリパスを表示
print(f"📁 入力フォルダ: {os.path.abspath(input_folder)}")
print(f"📁 一時フォルダ: {os.path.abspath(temp_folder)}")
print(f"📁 出力フォルダ: {os.path.abspath(output_folder)}")
print()
...
target_size = (750, 750)  # キャンバスサイズ
background_color = (255, 255, 255)  # 背景は白
content_target_size = 720  # 内容エリアの最大辺を650にリサイズ

# コマンド引数を入力（会場番号）
if len(sys.argv) < 2:
    print("=" * 50)
    print("🖼️  Layout 画像処理ツール")
    print("=" * 50)
    print("会場番号を入力してください（例: 7、12、123、1234）")
    print("注意: 画像は 0_input_images フォルダに入れてください")
    print()
    
    # 対話式入力（空入力かつ全ファイルが Layout_ で始まる場合はそのまま続行）
    while True:
        try:
            ...
            if user_input:
                ...
                if user_input.isdigit():
                    set_number = user_input  # 元の入力を保持
                    break
                else:
                    print("❌ 有効な数字を入力してください！")
            else:
                # 空入力時、入力フォルダ内が全て Layout_ で始まるかチェック
                def _all_prefixed():
                    def _scan(dir_path):
                        for item in os.listdir(dir_path):
                            ...
                            if os.path.isdir(item_path):
                                ...
                                if not item.startswith("Layout_"):
                                    return False
                        return True
                    return _scan(input_folder)

                if _all_prefixed():
                    print("全ての画像ファイル名が Layout_ で始まっていることを検出しました。元のファイル名のまま続行します…")
                    set_number = "0"  # ダミー、Layout_ プレフィックスのファイル名には使われません
                    break
                else:
                    print("❌ 会場番号が入力されておらず、Layout_ で始まらないファイルが存在するため続行できません。")
        except (EOFError, KeyboardInterrupt):
            print("\nプログラムを終了します")
            sys.exit(0)
    
    print(f"✅ 会場番号が設定されました: {set_number}")
    print()
else:
    set_number = str(sys.argv[1])  # 元の入力を保持
...
while True:
    ...
    print("全画像のトリミングとリサイズが完了しました。リネーム処理に進みます...")
    ...
    def get_new_name(filename):
        ...
        if ("T字島型" in name_for_check or "T字島" in name_for_check or "T字島形" in name_for_check or "T島型" in name_for_check or "T島形" in name_for_check) and has_projector:
        ...
        # 一般のパターンをチェック
        if "T字島型" in name_for_check or "T字島" in name_for_check or "T字島形" in name_for_check or "T島型" in name_for_check or "T島形" in name_for_check:
        ...
        if "正餐" in name_for_check or "着席" in name_for_check or "正座" in name_for_check or "着座" in name_for_check:
    ...
    for root, dirs, files in os.walk(temp_folder):
        ...
        for filename in files:
            ...
            if new_name == filename:
                ...
                print(f"{current_rel_path} -> ルールに一致しないため、処理されませんでした。")

    print("⭕️全画像のトリミング・リサイズ・リネーム処理が完了し、2_output_imagesに出力しました！")

    # 続けて実行しますか？（続行時は会場番号を再入力、空または無効時は0000を使用）
    while True:
        try:
                ...
                while True:
                    try:
                        ...
                        if rein and rein.isdigit():
                            ...
                            set_number = "0000"
                            print("有効な数字が入力されなかったため、デフォルト番号 0000 で続行します…")
                            break
                    except (EOFError, KeyboardInterrupt):
                        print("\nプログラムを終了します")
                        sys.exit(0)
