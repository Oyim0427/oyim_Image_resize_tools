...
print("=" * 50)
print("🖼️  Product SingleFood 画像処理ツール")
print("=" * 50)
...
while True:
    ...
    print(f"{len(image_files)} 枚の画像ファイルが見つかりました。")
    ...
    for file_path, filename, relative_path in image_files:
        ...
        try:
            ...
            if is_product:
                ...
                print(f"⭕️処理完了 (名前変更なし): {relative_path} -> {os.path.join(os.path.dirname(relative_path), output_filename)} ({processed.width}x{processed.height})")
            else:
                # ファイル名から情報を抽出
        ...
        for filename in files:
            ...
            for orig_path, keep_original in processed_files.items():
                ...
                if current_basename.startswith(orig_basename) or orig_basename.startswith(current_basename):
                    if keep_original:
                        print(f"出力完了 (名前変更なし): {current_rel_path}")
                    else:
    ...
    while True:
        try:
            ...
            if user_input in ['y', 'yes', '是', '继续']:
                ...
                # 再処理 - メインループに戻る
                break
            elif user_input in ['n', 'no', '否', '不', '退出']:
                print("👋 プログラムを終了します。ご利用ありがとうございました！")
                sys.exit(0)
            else:
                print("❌ y または n を入力してください")
